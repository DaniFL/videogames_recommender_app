...
const Navbar = () => {
    ...
    // ¡CAMBIO CLAVE! Obtenemos también 'isLoading' del contexto.
    ...
    // Cierra el menú si el usuario cambia (ej. al hacer logout)
    useEffect(() => {
        setMenuOpen(false);
    }, [user]);
                ...
                <div className="relative">
                    ...
                    {menuOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-[#283039] rounded-lg shadow-lg z-20">
                            {user.admin && (
                                <Link to="/admin/dashboard" className="block px-4 py-2 text-purple-400 font-bold hover:bg-[#111418] border-b border-gray-700">
                                        Panel de Admin
                                </Link>
                            )}
    ...
    const renderLinks = () => {
        // Durante la carga inicial, no mostramos ningún enlace de sesión para evitar el parpadeo.
        if (isLoading) {
            return <div className="h-8 w-24 bg-gray-700 rounded animate-pulse"></div>; // Placeholder de carga
        }
